...
def train_model(model, vf, optimizer, x, adj, adata_subsampled, epochs, run_dir, logger):
    ...
    # ABHI - commented out the below to try a new structural preservation technique
    ...
    # Abhi - cluster the data into the same number of nodes as the coarsened network, and into 2 dimensions
    G = nx.from_numpy_array(adj.numpy())
    pos = nx.spring_layout(G, seed=42)
    ...
    for i in range(epochs):
        ...
        # chain = match_chain_lengths(chain_coords, chain)
        ...
        # n = X.shape[0]
        # L_emb = torch.norm(X_origin - X, dim=0).mean() / np.sqrt(n)
