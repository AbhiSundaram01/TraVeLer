...
def monocle_pseudotime(adata, tsne):
    ...
    projected_points, mst, centroids = learn_graph(matrix=tsne, clusters=leiden)
...
def train_model(model, vf, optimizer, x, adj, adata_subsampled, epochs, run_dir, logger, λ_vf, Laplacian, λ_lap):
    ...
    for i in range(epochs):
        ...
        L_emb = OT_graph_similarity(tsne, x_out, P) 
...
λ_vf = 0
λ_laps = [0, 0.001, 0.01, 0.1, 1, 10, 100]
adata, tsne = preprocess_data(adata)
x, adj = get_initial_matrices(adata)

monocle_pseudotimes, root, umap = monocle_pseudotime(adata,)
...
#TODO
#tsne and umap need to come from adata_subsampled
#root needs to come from adata_tsne
#make sure cluster ratio --> 0.06 not 0.05
#Do the same to the other bone marrow experiment and re run