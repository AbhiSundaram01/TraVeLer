...
def train_model(model, vf, optimizer, x, adj, adata_subsampled, epochs, run_dir, logger, λ_vf, Laplacian, λ_lap):
    ...
    for i in range(epochs):
        ...
        L_local = local_constraint(umap, umap_nbrs, node_embeddings)
...
epochs = 150
seeds = [42]
correlation_sum = np.zeros((7,3))
for seed in seeds:
    set_seed(seed)
    correlations = np.zeros((7,3))
...
correlation_mean = correlation_sum / 1
