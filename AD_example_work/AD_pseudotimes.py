...
def main():
    ...
    wildtype = preprocess_data(wildtype)
    x, adj = get_initial_matrices(wildtype)
    ...
    umap_1 = wildtype.obs["umap_1"]
    umap_2 = wildtype.obs["umap_2"]
    ...
    losses, grad_norms_vf, grad_norms_model, x_sums = train_model(
        model, vf, optimizer, x, adj, wildtype, epochs, run_dir, logger, laplacian, umap)

    # Plot training metrics
    plot_training_metrics(losses, grad_norms_vf, grad_norms_model, x_sums, run_dir, logger)
    
    # Analyze cluster correspondence
    analyze_clusters(model, x, adj, wildtype, run_dir, logger)
    
    # Create embedding animations
    checkpoint_dir = f"{run_dir}/embedding_checkpoints"
    create_embedding_animation(checkpoint_dir, run_dir, logger)

    fig1 = visualize_joint_embeddings(model, x, adj, wildtype, full_hierarchy=True)
