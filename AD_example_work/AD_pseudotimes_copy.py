...
def setup_model(x, adj, logger):
    ...
    model = DirectedDiffPool(num_features=x.size(1), max_nodes=x.size(0),  cluster_ratio=0.0125)
...
def main():
    ...
    adata.obs["clusters"] = (adata.obs["cell.state_v2.1"].astype("category"))

    wildtype = adata[adata.obs["mouse.genotype"] == "APP-WT"]
    pathology = adata[adata.obs["mouse.genotype"] == "APP-NLGF"]

    pathology = preprocess_data(pathology)
    x, adj = get_initial_matrices(pathology)
    ...
    nbrs = NearestNeighbors(n_neighbors=101).fit(umap) #Reduce neighbours from 51 - 21
    ...
    losses, grad_norms_vf, grad_norms_model, x_sums = train_model(
        model, vf, optimizer, x, adj, pathology, epochs, run_dir, logger, laplacian, umap)

    # Plot training metrics
    plot_training_metrics(losses, grad_norms_vf, grad_norms_model, x_sums, run_dir, logger)
    
    # Analyze cluster correspondence
    analyze_clusters(model, x, adj, pathology, run_dir, logger)
    
    # Create embedding animations
    checkpoint_dir = f"{run_dir}/embedding_checkpoints"
    create_embedding_animation(checkpoint_dir, run_dir, logger)

    fig1 = visualize_joint_embeddings(model, x, adj, pathology, full_hierarchy=True)
...
##change cluster ratios for pathology and wildtype respectively:
##wildtype: 39k - 8: 0.015
##pathology: 53k - 8: 0.0125

##Could maybe try looking at larger laplacian neighbour numbers - 101