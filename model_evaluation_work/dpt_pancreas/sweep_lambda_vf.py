...
LAMBDA_LAP = 100        # fixed from lambda_lap sweep (evaluation.py)
...
def setup_model(x: torch.Tensor, adj: torch.Tensor):
    model = DirectedDiffPool(num_features=x.size(1), max_nodes=x.size(0), cluster_ratio=0.05, output_dim=10)
    c = 1
    vf_in = nn.Sequential(
