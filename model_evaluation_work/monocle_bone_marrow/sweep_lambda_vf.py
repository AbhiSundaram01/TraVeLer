"""
Monocle Bone Marrow — Lambda VF Sweep
========================================
...
def setup_run(label: str = "monocle_bone_marrow_vf_sweep"):
...
def main():
    _, run_dir, logger = setup_run("monocle_bone_marrow_vf_sweep")
    ...
    Laplacian = build_umap_laplacian(X_umap, n_neighbors=50)
    ...
    plot_correlation_vs_lambda_vf(
        ...
        f"Monocle Bone Marrow ($\\lambda_{{lap}}={LAMBDA_LAP}$, {len(SEEDS)} seeds)",
    )
